...
def set_frequency_range():
    """ Apply new frequency range and restart HackRF """
    global freq_low, freq_high, total_freq_bins, frequencies, waterfall_data, power_values, total_lines, max_hold

    try:
        # Get the frequency range from the user input
        freq_low = int(freq_low_input.text())
        freq_high = int(freq_high_input.text())

        # Ensure valid range
        if freq_low < 0 or freq_high > 6000 or freq_low >= freq_high:
            print(f"Invalid frequency range: {freq_low}-{freq_high} MHz")
            return
        ...
        print('Invalid Frequency Range Input')
...
freq_low_input = QLineEdit()
freq_low_input.setText(str(freq_low))
freq_high_input = QLineEdit()
freq_high_input.setText(str(freq_high))
...
ui_layout.addWidget(QLabel('High Frequency:'))
ui_layout.addWidget(freq_high_input)
ui_layout.addWidget(QLabel('Low Frequency:'))
ui_layout.addWidget(freq_low_input)
...
try:
    ...
    log_file = open("Tests/test3_operator_unna_drone_flyr_halvsirkel_konstant_avstand_52.csv", "a")
    def read_data():
