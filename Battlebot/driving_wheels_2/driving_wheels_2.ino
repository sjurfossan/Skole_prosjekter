...
void loop() {
    ...
    //right motor
    if (R_stick > middle_ch + 50) // right motor forward
    {
      analogWrite(ENABLE_R, velocity_R);
      digitalWrite(DIR_R, HIGH);
      digitalWrite(DIR_R_B, LOW);
      velocity_pos_R = R_stick - middle_ch; // is for report the velocity in the same intervall of the negative velocity
      velocity_R = constrain(velocity_pos_R,0 , 255);
    }
    
    
    // right motor Backward
    else if  (R_stick < (middle_ch - 50)) 
    {
      analogWrite(ENABLE_R, velocity_R);
 ...
 //left motor
    if (L_stick > (middle_ch + 50)) // left motor forward
    {
      analogWrite(ENABLE_L, velocity_L); 
      digitalWrite(DIR_L, HIGH);
      digitalWrite(DIR_L_B, LOW);
      velocity_pos_L = L_stick - middle_ch; // is for report the velocity in the same intervall of the negative velocity
      velocity_L = constrain(velocity_pos_L,0 , 255);
    }

    // left motor Backward
    else if (L_stick < (middle_ch - 50)) 
    {
      analogWrite(ENABLE_L, velocity_L); 
    ...
    //long int time_2 = millis();
    
    //Serial.print("PulseIn time: "); Serial.print(time_2-time_1); Serial.println(" milliseconds");
  }
